<?php
/**
 * Common used functions of the system.
 * @author LW
 * @since  0.1.1
 */
...
/**
 * Get the GMT time according to the date set
 ...
 * @param $utc_time
 * @param $format time format , default : Y-m-d H:i:s
 * @return string
 */
 ...
 * 保存图片
 * 
 * @param array $upd_file
 *        	即上传的$_FILES数组
 * @param array $key
 *        	$_FILES 中的键名 为空则保存 $_FILES 中的所有图片
 * @param string $dir
 *        	保存到的目录
 * @param array $whs
 *        	可生成多个缩略图
 ...
 * @param $id //数据ID        	
 * @param $field //
 *        	tag_match/name_match/cate_match/locate_match
 */
...
function send_user_password_mail($user_id) {
	...
	if ($user_info) {
		...
		$user_info ['password_url'] = get_domain () . url ( "settings#password", array (
				"code" => $user_info ['password_verify'],
				"id" => $user_info ['id'] 
		) );
		...
		if (! $order_info ['is_tg']) {
			...
			if ($order_info ['score'] > 0) {
				...
				modify_account ( array (
						"money" => $order_info ['score_money'],
						"score" => "-" . $order_info ['score'] 
				), $order_info ['user_id'], $log_score, array (
						'money_type' => 20 
				) );
			}
			if ($order_info ['type'] != 6) {
				...
				), $order_info ['user_id'], $order_info ['deal_name'] . "购买成功", array (
						'money_type' => 16,
				...
				$re = modify_account ( array (
						"money" => "-" . $order_info ['total_price'] 
				), $order_info ['user_id'], $order_info ['deal_name'] . "股权转让支付成功", array (
						'money_type' => 16,
						'deal_id' => $order_info ['deal_id'],
						'order_id' => $order_info ['id'] 
				) );
		...
		if ($GLOBALS ['db']->affected_rows () > 0) {
			...
			if ($GLOBALS ['db']->affected_rows () > 0 || ($order_info ['type'] == 1 || $order_info ['type'] == 5)) {
				...
				if ($order_info ['type'] != 6) {
					...
					$GLOBALS ['msg']->manage_msg ( 'MSG_PAID', $order_info ['user_id'], array (
							'money' => $order_info ['total_price'],
							'order_id' => $order_info ['id'] 
					) );
					// 更新用户的支持数
				...
				modify_account ( array (
						"money" => ($order_info ['online_pay'] + $order_info ['credit_pay']) 
				), $order_info ['user_id'], $order_info ['deal_name'] . "限额已满，转存入会员帐户" );
				// 退回积分
				if ($order_info ['score'] > 0) {
					$log_score = $order_info ['deal_name'] . "限额已满，退回" . $order_info ['score'] . "积分";
					modify_account ( array (
							"score" => $order_info ['score'] 
					), $order_info ['user_id'], $log_score );
				}
			}
		} else {
			$result ['status'] = 1;
			...
			modify_account ( array (
					"money" => ($order_info ['online_pay'] + $order_info ['credit_pay']) 
			), $order_info ['user_id'], $order_info ['deal_name'] . "已过期，转存入会员帐户" );
			// 退回积分
			if ($order_info ['score'] > 0) {
				$log_score = $order_info ['deal_name'] . "限额已满，退回" . $order_info ['score'] . "积分";
				modify_account ( array (
...
function send_notify($user_id, $content, $url_route, $url_param) {
	$GLOBALS ['msg']->manage_msg ( "notify", $user_id, array (
			'content' => $content,
			'url_route' => $url_route,
			'url_param' => $url_param 
	) );
...
function send_verify_sms($mobile, $code, $type = "") {
	$GLOBALS ['msg']->manage_msg ( 'TPL_SMS_VERIFY_CODE', $mobile, array (
			'code' => $code,
			'title' => $title 
	) );
}
...
function send_tzt_verify_sms($mobile, $code) {
	$GLOBALS ['msg']->manage_msg ( 'TPL_SMS_TZT_VERIFY_CODE', $mobile, array (
			'code' => $code,
...
function send_verify_email($email, $code, $title = "") {
	$GLOBALS ['msg']->manage_msg ( 'TPL_MAIL_USER_VERIFY', $email, array (
			'code' => $code,
			'title' => $title 
	) );
}
...
function send_investor_status($user_info) {
	if ($user_info ['id']) {
		$GLOBALS ['msg']->manage_msg ( "MSG_INVEST_STATUS", $user_info ['id'], array (
				'user_info' => $user_info 
		) );
	}
}
...
function invest_pay_send($invest_id, $order_id = '') {
	$GLOBALS ['msg']->manage_msg ( "MSG_INVESTOR_GO_PAY", '', array (
			'invest_id' => $invest_id,
			'order_id' => $order_id 
	) );
	...
	if (FALSE) {
		...
		if ($status == 1 && $user_info) {
			...
			if (! empty ( $user_info ['email'] )) {
				...
				$user_info ['site_name'] = app_conf ( "SITE_NAME" );
				if ($order_id) {
					$user_info ['note_url'] = get_domain () . url ( "account#view_order", array (
							'id' => $order_id 
					) );
				} else {
...
function send_paid_info($notice_id) {
	...
	if ($status > 0) {
		$user_info = $GLOBALS ['db']->getRow ( "select pn.id as notice_id,pn.user_id,pn.money,u.user_name,u.mobile,u.email,pn.money as paid_money,pn.notice_sn from " . DB_PREFIX . "payment_notice as pn " . "left join " . DB_PREFIX . "user as u on u.id=pn.user_id " . " where pn.id=$notice_id and pn.is_paid = 1 and pn.paid_send=0" );
...
function set_mortgate($user_id, $deal_id, $money) {
	...
	if ($need_mortgate == ($has_money + $money)) {
		...
		if ($GLOBALS ['db']->insert_id ()) {
			modify_account ( array (
					'money' => '-' . $money 
			), $user_id, "项目ID" . $deal_id . " 冻结诚意金", $param = array () );
		}
		return true;
	} else {
		return false;
...
function investor_leader_ajax($user_id, $deal_id, $ajax = '', $from = 'web', $info, $type = '') {
	...
	if ($info ['is_investor'] == 0) {
		...
		if ($from == 'web') {
			$result ['url'] = url ( "settings#security", array (
					'method' => 'setting-id-box' 
			) );
		} elseif ($from == 'wap') {
		...
		if (! $invest) {
			...
			if ($from == 'web') {
				if ($type != '') {
					$result ['url'] = url ( "investor#applicate_leader", array (
							"deal_id" => $deal_id,
							"type" => $type 
					) );
				} else {
				...
				$result ['url'] = url_wap ( "investor#applicate_leader", array (
						"deal_id" => $deal_id 
				) );
			} elseif ($from == 'app') {
		...
		if ($invest ['status'] == 1) {
			...
			if ($margator_money >= user_need_mortgate ()) {
				...
				if ($from == 'web') {
					$result ['url'] = url ( "account#mortgate_pay", array (
							'deal_id' => $deal_id 
					) );
				} elseif ($from == 'wap') {
					$result ['url'] = url_wap ( "account#mortgate_pay", array (
							'deal_id' => $deal_id 
					) );
				} elseif ($from == 'app') {
...
function investor_save_leader($deal_id, $user_id, $cates, $investor_id, $invest, $introduce, $from = 'web', $type = '') {
	if ($investor_id > 0) {
		...
		if ($GLOBALS ['db']->query ( "UPDATE " . DB_PREFIX . "investment_list set introduce='" . $introduce . "',cates='" . $cates . "',create_time=" . NOW_TIME . ",status=" . $invest ['status'] . " where id=$investor_id " )) {
			...
			if ($from == 'web') {
				if ($type == 4) {
					$data ['url'] = url ( "finance#company_finance", array (
							"id" => $deal_id 
					) );
				} else {
					$data ['url'] = url ( "deal#show", array (
							"id" => $deal_id 
					) );
				}
			} elseif ($from == 'wap') {
				$data ['url'] = url_wap ( "deal#show", array (
						"id" => $deal_id 
				) );
			}
			return $data;
...
function set_nav_top($module, $action = 'index', $id = 0) {
	if ($module == 'article_cate' && $action == 'index' && $id == 0) {
		...
		return array (
				...
				'cate_child' => array (
						'name' => $cate_name,
						'url' => url ( 'article_cate#index', array (
								'id' => $id 
						) ) 
				) 
		);
...
function get_admin_nav($role_id, $adm_name) {
	if ($adm_name == app_conf ( 'DEFAULT_ADMIN' )) {
		...
		$navs = load_auto_cache ( "admin_nav", array (
				'id' => $role_id 
		) );
	}
	return $navs;
...
function buildPage($type, $args, $total_count, $page = 1, $page_size = 0, $url = '', $offset = 5) {
	...
	if ($pager ['page_count'] <= $offset * 2) {
		for($i = 1; $i <= $pager ['page_count']; $i ++) {
			$pager ['page_nums'] [] = array (
					'name' => $i,
					'url' => empty ( $url ) ? str_replace ( '_page_', $i, $page_url ) : $url . $i 
			);
		}
	} else {
		if ($pager ['page'] - $offset < 2) {
			...
			$pager ['page_nums'] [] = array (
					'name' => $pager ['page_count'],
					'url' => empty ( $url ) ? str_replace ( '_page_', $pager ['page_count'], $page_url ) : $url . $pager ['page_count'] 
			);
		} else {
			$pager ['page_nums'] [] = array (
					'name' => 1,
					'url' => empty ( $url ) ? str_replace ( '_page_', 1, $page_url ) : $url . 1 
			);
			$pager ['page_nums'] [] = array (
					'name' => '...' 
			);
...
function show_avatar($u_id, $type = "middle", $head_img = '') {
	...
	if (isset ( $GLOBALS [$key] )) {
		...
		if (! isset ( $avatar_data ) || ! isset ( $avatar_data [$key] )) {
			...
			$avatar_str = "<a href='" . url ( "home", array (
					"id" => $u_id 
			) ) . "' style='text-align:center; display:inline-block;'>" . "<img src='" . $avatar_file . "'  />" . "</a>";
...
function check_url($url) {
	$patern = '/^http[s]?:\/\/' . '(([0-9]{1,3}\.){3}[0-9]{1,3}' . // IP形式的URL- 199.194.52.184
'|' . // 允许IP和DOMAIN（域名）
'([0-9a-z_!~*\'()-]+\.)*' . // 域名- www.
