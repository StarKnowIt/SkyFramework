...
//		}
		...
		//新增会员信息		
		$user['asset']=strim($user_data['asset']);
		$user['office_documents']=strim($user_data['office_documents']);
		$user['guarantee']=strim($user_data['guarantee']);
		
		$user['authorized_operation_letter']=strim($user_data['authorized_operation_letter']);
		$user['authorized_operation_certificate']=strim($user_data['authorized_operation_certificate']);
		...
		$user['member_type']=strim($user_data['member_type']);
		//会员状态
		...
		if(floatval($data['money'])!=0 || $score !=0 || $point !=0||floatval($data['mortgage_money'])!=0 )
		{
			...
			if($GLOBALS['db']->insert_id()){
				if($param[money_type]==17){
					...
					//接收筹款
					$GLOBALS['msg']->manage_msg("notify",$user_id,array('content'=>$log_msg,'url_route'=>$param['url_route'],'url_param'=>$param['url_param']));
				}
			}
