...
function payment_paid($notice_sn,$outer_sn)
{
	...
	if($payment_notice)
	{
		...
		{
			//充值单
			$GLOBALS['db']->query("update ".DB_PREFIX."payment_notice set pay_time = '".get_gmtime()."',is_paid = 1,outer_notice_sn = '".$outer_sn."' , pay_date = '".to_date(get_gmtime(), 'Y-m-d')."' where id = ".$payment_notice['id']." and is_paid = 0");
