	...
	function __construct() {
		if (file_exists ( APP_ROOT_PATH . "public/install.lock" )) {
			...
			if ($this->is_wx) {
				...
				if ($account) {
					$this->wx_tmpl = load_auto_cache ( "weixin_tmpl", array (
	...
	public function wx_content_code($tmpl) {
		if ($this->wx_tmpl [$tmpl] && $this->user_info ['wx_openid']) {
			...
			if ($tmpl == 'OPENTM201490080') {
				...
				$data ['keyword1'] = array (
						'value' => $this->user_info ['notice_sn'],
						'color' => '#173177' 
				);
				$data ['keyword2'] = array (
						'value' => $this->user_info ['deal_name'],
						'color' => '#173177' 
				);
				$data ['keyword3'] = array (
						'value' => format_price ( $this->user_info ['paid_money'] ),
						'color' => '#173177' 
				);
				$wx_url = get_domain () . url_wap ( "account#view_order", array (
						'id' => $this->user_info ['notice_sn'] 
				) );
			} elseif ($tmpl == 'OPENTM206915380') {
				// 调用OPENTM206915380 ,后台管理操作通知
				$data ['keyword1'] = array (
						'value' => $this->user_info ['control_type'],
						'color' => '#173177' 
				);
				$data ['keyword2'] = array (
						'value' => $now_date,
				...
				$data ['keyword3'] = array (
						'value' => $this->user_info ['control_content'],
						'color' => '#173177' 
				);
				$data ['keyword4'] = array (
						'value' => '处理完成',
				...
				$data ['keyword1'] = array (
						'value' => $this->user_info ['wx_code'],
						'color' => '#173177' 
				);
				$data ['keyword2'] = array (
						'value' => $this->user_info ['expire_time'],
						'color' => '#173177' 
				);
			} elseif ($tmpl == 'OPENTM267386236') {
				$data ['keyword1'] = array (
						'value' => format_price ( $this->user_info ['incharge_money'] ),
						'color' => '#173177' 
				);
				$data ['keyword2'] = array (
						'value' => $now_date,
				...
				$data ['keyword2'] = array (
						'value' => format_price ( $this->user_info ['refund_money'] ),
						'color' => '#173177' 
				);
			} elseif ($tmpl == 'TM00979') {
				// 提现申请通知
				...
				$data ['money'] = array (
						'value' => format_price ( $this->user_info ['carray_money'] ),
						'color' => '#173177' 
				);
				$data ['timet'] = array (
						'value' => $now_date,
				...
				$data ['keyword2'] = array (
						'value' => $this->user_info ['carray_bank'],
						'color' => '#173177' 
				);
				$data ['keyword3'] = array (
						'value' => $this->user_info ['carray_create_time'],
						'color' => '#173177' 
				);
				$data ['keyword4'] = array (
						'value' => $this->user_info ['carray_end_time'],
						'color' => '#173177' 
				);
				$data ['keyword5'] = array (
						'value' => $this->user_info ['result'],
						'color' => '#173177' 
				);
			} elseif ($tmpl == 'OPENTM202116678') {
				// 众筹通知
				$data ['keyword1'] = array (
						'value' => $this->user_info ['control_title'],
						'color' => '#173177' 
				);
				$data ['keyword2'] = array (
						'value' => $this->user_info ['control_content'],
