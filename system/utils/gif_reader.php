...
class GIFReader {
				...
				case 33 :
					...
					switch (ord ( $this->BUF {$seek} )) {
							...
							if (count ( $this->IMGS ["frames"] ) == 0) {
								...
								if ($gifString == "NETSCAPE") {
									...
									if ($gByte == 3) {
										...
										$seek += 2;
										if ($this->IMGS ["info"] ["Loops"] < 1)
											$this->IMGS ["info"] ["Loops"] = 0;
									} else {
										//
									}
								} else {
						...
						case 249 :
							...
							if ($tmp ["FrameDisposal"] == 3)
								$this->IMGS ["info"] ["UsesBkBuffer"] = True;
	...
	function getgif($i) {
		if (! isset ( $this->IMGS ["frames"] [$i] ["byteOffset(Begin)"] ))
			return "";
	...
	function getheader() {
		if (! isset ( $this->IMGS ["frames"] [0] ["byteOffset(Begin)"] ))
			return "";
